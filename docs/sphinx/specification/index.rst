...
Schema version
  Version number of the schema; year-month in the form ``YYYY-MM`` for
  all releases from 2007 and later.

Documentation
  Documentation of each schema element and attribute, automatically
  generated from the schema.

Schema URIs
  URI for each schema file in the general form
  ``http://www.openmicroscopy.org/Schemas/<namespace>/<version>/<namespace>.xsd``

Transforms
  URIs for XSL transforms to transform between different schema
  versions ("upgrading" and "downgrading").

Changes
  Key changes for each release. Some changes affect the structure of
  the data model, some only the names used for concepts, and others
  only the XSD files themselves.  Note that the XSD files are
  versioned using the ``schema`` element ``version`` attribute.  Here,
  *version 1* refers to the initial release of a new schema version,
  and higher versions refer to updates to this schema version where
  the ``version`` in individual schemas as been incremented for a
